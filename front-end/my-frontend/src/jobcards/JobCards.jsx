        ...
        {filteredJobCards.map(job => (
          <div key={job.id} className="bg-white rounded-lg shadow-lg hover:shadow-xl transition-shadow p-6">
            ...
            <div className="space-y-2 text-sm">
              ...
              {job.barcode && ( // ← ADD THIS: Show barcode if available
                <div className="flex justify-between">
                  <span className="text-gray-600">Barcode:</span>
                  <span className="font-medium text-gray-900">{job.barcode}</span>
                </div>
              )}
              <div className="flex justify-between">
                <span className="text-gray-600">Estimated Cost:</span>
                <span className="font-medium text-green-600">Rs.{job.estimatedCost?.toFixed(2) || '0.00'}</span>
              </div>
            </div>
