...
//     // Add CHARGER to components if withCharger is true
//     if (jobCard.withCharger) {
//       if (!uniqueSerialTypes.includes('CHARGER')) {
//         uniqueSerialTypes.push('CHARGER');
//       }
//     }
...
//     <div class="terms-item">Job note is essential to release the device will not be released at all without job note.</div>
//     <div class="terms-item">Inspection fee Rs.1000 will be charged if the device is carried out without repairing.</div>
//     <div class="terms-item">We will expect to complete all the repairs as soon as we can after final inspection will inform you to about device condition to provided phone number. If the device is not collected within 3 months of the repair, it would be disposed to recover the cost.</div>
//     <div class="terms-item">We will provide 2-month warranty on repairs carried out by us and this applies only to defects which we have rectified.</div>
//     <div class="terms-item">We will not take any responsibilities regarding data in your computer and this will be confirmed by us when you hand over the device to repair.</div>
// </div>
...
//           {/* With Charger Information - Updated for PDF integration */}
//           <div className="border-b border-gray-200 pb-6">
//             <h2 className="text-xl font-semibold text-gray-900 mb-4 flex items-center">
...
//                   jobCard.withCharger ? 'bg-green-600' : 'bg-gray-400'
//                 }`}>
//                   {jobCard.withCharger ? (
...
//                     jobCard.withCharger ? 'text-green-900' : 'text-gray-900'
//                   }`}>
//                     {jobCard.withCharger ? '✓ With Charger' : '✗ Without Charger'}
//                   </h3>
//                   <p className={`text-sm mt-1 ${jobCard.withCharger ? 'text-green-700' : 'text-gray-600'}`}>
//                     {jobCard.withCharger 
//                       ? 'Device received with charger - will be returned with device'
...
//           {/* Components Information (for web view) */}
...
//                   {otherSerials.map((serial, index) => (
//                     <div key={index} className="flex items-center gap-2 bg-indigo-200 text-indigo-800 px-3 py-1 rounded-full">
//                       <span className="font-medium">{serial.serialType.replace(/_/g, ' ')}</span>
//                     </div>
//                   ))}
//                   {jobCard.withCharger && (
//                     <div className="flex items-center gap-2 bg-green-200 text-green-800 px-3 py-1 rounded-full">
...
//       setTimeout(() => msg.remove(), 3000);
//       setTimeout(() => {
//         try { if (onStatusChange) onStatusChange(); } catch (err) { console.error('Error in onStatusChange callback:', err); }
//         setTimeout(() => { if (onClose) onClose(); }, 1000);
//       }, 500);
//     } catch (err) { console.error('Error in handleCancelSuccess:', err); setShowCancelModal(false); }
//   };
...
//       setTimeout(() => msg.remove(), 3000);
//       if (onStatusChange) onStatusChange();
//     } catch (err) { setError(err.message || 'Failed to update status'); } finally { setUpdatingStatus(false); }
//   };
...
//   const calcServiceTotal = () => (jobCard?.serviceCategories || []).reduce((sum, s) => sum + (s.servicePrice || 0), 0);
...
//         jobCard.status === 'CANCELLED' ? 'bg-gray-700' :
//         'bg-blue-700'
//       }`}>
...
//         {/* COLUMN 1: Customer + Device + Timeline */}
//         <div className="flex flex-col gap-2 overflow-hidden">
//           {/* Customer */}
//           <div className="border border-gray-200 rounded p-2 bg-white">
//             <div className="text-xs font-bold text-gray-700 mb-1.5 uppercase tracking-wide">Customer</div>
...
//         {/* COLUMN 2: Serials + Conditions + Faults */}
//         <div className="flex flex-col gap-2 overflow-hidden">
//           {/* Device Serials */}
//           {deviceSerials.length > 0 && (
...
//         {/* COLUMN 3: Services + Used Items */}
//         <div className="flex flex-col gap-2 overflow-hidden">
//           {/* Services */}
//           {jobCard.serviceCategories && jobCard.serviceCategories.length > 0 && (
//             <div className="border border-green-200 rounded p-2 bg-green-50">
//               <div className="text-xs font-bold text-green-700 mb-1.5 uppercase tracking-wide">Services ({jobCard.serviceCategories.length})</div>
//               <div className="flex flex-wrap gap-1 mb-1.5 max-h-20 overflow-y-auto">
//                 {jobCard.serviceCategories.map(s => (
//                   <span key={s.id} className="bg-green-200 text-green-800 px-1.5 py-0.5 rounded-full text-xs font-medium">
//                     {s.name} - Rs.{s.servicePrice?.toFixed(2) || '0.00'}
//                   </span>
//                 ))}
//               </div>
...
//         {/* COLUMN 4: Payment + Status Actions + Buttons */}
//         <div className="flex flex-col gap-2 overflow-hidden">
//           {/* Payment */}
//           <div className="border border-gray-200 rounded p-2 bg-white">
...
//               </div>
//               <div className="flex justify-between items-center p-1.5 bg-purple-50 border border-purple-200 rounded">
//                 <span className="text-xs text-gray-600">Total (now)</span>
//                 <span className="text-sm font-bold text-purple-700">Rs.{calcGrandTotal().toFixed(2)}</span>
//               </div>
//             </div>
//           </div>
...
//   const [showCreateInvoiceModal, setShowCreateInvoiceModal] = useState(false);
//   const [updatingStatus, setUpdatingStatus] = useState(false);
//   // ── NEW: track whether an invoice exists for this job card ──
...
//         const data = await apiCall(`/api/jobcards/${jobCardId}`);
//         setJobCard(data);
//         // Hide cancel button if the job card already has an invoice linked
...
//   const handleInvoiceSuccess = (response) => {
//     setShowCreateInvoiceModal(false);
//     // ── NEW: mark invoice as generated → hides Cancel Job Card button ──
//     setInvoiceGenerated(true);
//     const msg = document.createElement('div');
//     msg.className = 'fixed top-4 right-4 bg-green-500 text-white px-4 py-2 rounded-lg shadow-lg z-50 text-sm';
//     msg.textContent = 'Invoice created successfully!';
...
//   // ── NEW: Cancel button hidden when already cancelled, delivered, OR invoice generated ──
...
//           </span>
//           {jobCard.oneDayService && <span className="px-2 py-0.5 bg-white text-red-600 rounded-full text-xs font-bold">🚨 ONE DAY</span>}
//           {/* ── NEW: invoice badge in header ── */}
//           {invoiceGenerated && (
//             <span className="px-2 py-0.5 bg-white text-purple-700 rounded-full text-xs font-bold">📄 Invoice Generated</span>
//           )}
//         </div>
//         <div className="flex items-center gap-2">
//           {error && <span className="text-xs text-red-200 bg-red-800 rounded px-2 py-0.5">{error}</span>}
...
//                   </button>
//                 )}
//                 {/* ── Cancel button: hidden once invoice is generated ── */}
//                 {showCancelButton && (
//                   <button onClick={() => setShowCancelModal(true)} className="w-full py-1.5 bg-red-600 hover:bg-red-700 text-white text-xs font-medium rounded transition-colors">
//                     ✕ Cancel Job Card
//                   </button>
...
const COMPANY_LOGO_BASE64 = 'data:image/jpeg;base64,';
