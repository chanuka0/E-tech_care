...
const InvoiceList = () => {
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      {/* ✅ UPDATED: 5-column stats — returned invoices excluded from blue/green/red, new orange box */}
      {filteredByStatus.length > 0 && (
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          ...
          <div className="bg-white p-4 rounded-lg shadow border-l-4 border-orange-500">
            <p className="text-xs text-orange-600 font-medium mb-1">Total Returned</p>
            <p className="text-2xl font-bold text-orange-600">
              Rs.{filteredByStatus
                .filter(inv => inv.isReturned)
                .reduce((sum, inv) => sum + (inv.returnedAmount ?? inv.paidAmount ?? 0), 0)
                .toFixed(2)}
            </p>
          </div>
        </div>
