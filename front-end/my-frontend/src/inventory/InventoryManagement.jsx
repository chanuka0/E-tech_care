...
const InventoryManagement = () => {
  ...
  // UPDATED: Check if item can be deleted - only for newly created items with no activity
  const canDeleteItem = (item) => {
    // Item can be deleted only if:
    // 1. Has zero quantity (no stock added)
    // 2. No serial numbers added
    // 3. Never had any stock movements (we'll check if it's essentially "new")
    
    const hasStock = item.quantity > 0;
    const hasSerials = item.serials && item.serials.length > 0;
    const hasAnyActivity = hasStock || hasSerials;
                            ...
                            <div className="flex space-x-2">
                              ...
                              {isAdmin() && (
                                <>
                                  ...
                                  {/* DELETE BUTTON - Only show when item can be deleted */}
                                  {canDelete && (
                                    <button
                                      onClick={() => handleDeleteItem(item.id)}
                                      className="text-red-600 hover:text-red-900 font-medium"
