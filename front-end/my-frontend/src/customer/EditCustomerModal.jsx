...
// const EditCustomerModal = ({ customer, onUpdate, onClose, existingCustomers }) => {
...
//       c.phoneNumber === formData.phoneNumber && c.customerId !== customer.customerId
//     )) {
...
//     } else if (existingCustomers.some(c => 
//       c.email === formData.email && c.customerId !== customer.customerId
//     )) {
...
const EditCustomerModal = ({ customer, onUpdate, onClose, existingCustomers }) => {
  ...
  const validateForm = () => {
    ...
    else if (existingCustomers.some(c => c.phoneNumber === formData.phoneNumber && c.customerId !== customer.customerId)) e.phoneNumber = 'Phone number already exists';
    if (!formData.email.trim()) e.email = 'Email is required';
    else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.email)) e.email = 'Invalid email';
    else if (existingCustomers.some(c => c.email === formData.email && c.customerId !== customer.customerId)) e.email = 'Email already exists';
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try { await onUpdate(formData); }
    catch (err) { setErrors({ submit: err.message || 'Failed to update customer' }); }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md flex flex-col">
        ...
        <div className="bg-blue-600 px-4 py-3 flex items-center justify-between flex-shrink-0 rounded-t-lg">
          <h3 className="text-base font-bold text-white">Edit Customer</h3>
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-2.5">
          ...
          <div className="flex gap-2 pt-2 border-t border-gray-200">
            ...
            <button type="submit" disabled={loading} className="flex-1 px-4 py-2 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white text-sm font-medium rounded-lg">{loading ? 'Updating...' : 'Update Customer'}</button>
          </div>
        </form>
      </div>
    </div>
