...
const EditModelModal = ({ model, onUpdate, onClose, brands, existingModels }) => {
  const [formData, setFormData] = useState({
    brand: { id: '' },
    modelName: '',
    description: '',
  ...
  // Frontend duplicate checking (excluding current model)
  const checkForDuplicates = () => {
    if (!formData.brand.id || !formData.modelName) {
      return null; // Let required validation handle this
    }

    const selectedBrandId = parseInt(formData.brand.id);
    const modelName = formData.modelName;

    // Find the selected brand name for error message
    const selectedBrand = brands.find(b => b.id === selectedBrandId);
    const brandName = selectedBrand?.brandName || 'Selected Brand';

    // Check if this exact combination already exists (excluding current model)
    const duplicate = existingModels.find(m => 
      m.id !== model.id && // Exclude current model
      m.brand?.id === selectedBrandId && 
      m.modelName === modelName
    );

    if (duplicate) {
      return `A model with the name '${modelName}' already exists for brand '${brandName}'. The same model name cannot be added to the same brand twice.`;
    }
