...
const ModelNumberManagement = () => {
  ...
  // CRUD Operations
  const handleAddModelNumber = async (newModelNumber) => {
    console.log('🔄 Adding model number:', newModelNumber);
    ...
    try {
      ...
      console.log('✅ Model number added:', response);
      setModelNumbers([...modelNumbers, response]);
      setShowAddModal(false);
      showSuccessMessage('Model number added successfully!');
    } catch (err) {
      console.error('❌ Failed to add model number:', err);
      // Re-throw error to be handled by the modal
      throw new Error(err.message || 'Failed to add model number');
    }
  };
  ...
  const handleUpdateModelNumber = async (updatedModelNumber) => {
    try {
      ...
      // Re-throw error to be handled by the modal
      throw new Error(err.message || 'Failed to update model number');
    }
  };

  const handleDeleteModelNumber = async (id) => {
    if (window.confirm('⚠️ Are you sure you want to permanently delete this model number?\n\nThis action cannot be undone and will remove it from the database.')) {
      try {
        await apiCall(`/api/model-numbers/${id}`, {
          method: 'DELETE'
        });
        setModelNumbers(modelNumbers.filter(mn => mn.id !== id));
        showSuccessMessage('Model number deleted permanently!');
      } catch (err) {
        setError(err.message || 'Failed to delete model number');
      }
    }
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        <div className="flex items-start space-x-3">
          ...
          <div className="text-sm text-blue-800">
            <p className="font-semibold mb-1">Model Number Rules:</p>
            <ul className="list-disc list-inside space-y-1">
              <li>The <strong>same model</strong> cannot have duplicate model numbers</li>
              <li><strong>Different models</strong> can share the same model number</li>
              <li>Deleting a model number will <strong>permanently remove</strong> it from the database</li>
            </ul>
          </div>
        </div>
      </div>
