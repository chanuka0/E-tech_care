...
const EditModelNumberModal = ({ modelNumber, onUpdate, onClose, models, existingModelNumbers }) => {
  ...
  // Frontend duplicate checking (excluding current model number)
  const checkForDuplicates = () => {
    ...
    const modelNumberValue = formData.modelNumber;

    // Find the selected model name for error message
    const selectedModel = models.find(m => m.id === selectedModelId);
    const modelName = selectedModel?.modelName || 'Selected Model';

    // Check if this exact combination already exists (excluding current model number)
    const duplicate = existingModelNumbers.find(mn => 
      mn.id !== modelNumber.id && // Exclude current model number
      mn.model?.id === selectedModelId && 
      mn.modelNumber === modelNumberValue
    );

    if (duplicate) {
      return `Model number '${modelNumberValue}' already exists for model '${modelName}'. The same model number cannot be added to the same model twice.`;
    }
