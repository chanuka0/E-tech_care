...
const DeviceConditionManagement = () => {
  ...
  // ✅ MODIFIED: Added confirmation before activating
  const handleActivate = async (condition) => {
    const confirmActivate = window.confirm(
      `Are you sure you want to activate the device condition "${condition.conditionName}"?\n\nThis condition will be available for new job cards.`
    );
    ...
    try {
      const response = await apiCall(`/api/device-conditions/${condition.id}/activate`, {
        method: 'PATCH'
      });
      setConditions(conditions.map(c => c.id === condition.id ? response : c));
      showSuccessMessage(`${condition.conditionName} activated successfully!`);
    } catch (err) {
      setError(err.message || 'Failed to activate device condition');
    }
  };

  // ✅ MODIFIED: Added confirmation before deactivating
  const handleDeactivate = async (condition) => {
    const confirmDeactivate = window.confirm(
      `Are you sure you want to deactivate the device condition "${condition.conditionName}"?\n\nThis condition will not be available for new job cards.`
    );
    ...
    try {
      const response = await apiCall(`/api/device-conditions/${condition.id}/deactivate`, {
        method: 'PATCH'
      });
      setConditions(conditions.map(c => c.id === condition.id ? response : c));
      showSuccessMessage(`${condition.conditionName} deactivated successfully!`);
    } catch (err) {
      setError(err.message || 'Failed to deactivate device condition');
    }
  };
                ...
                {filteredConditions.map(condition => (
                  <tr key={condition.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                     ...
                     <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-600">
                        <div>{new Date(condition.createdAt).toLocaleDateString()}</div>
                        <div className="text-xs text-gray-500">
                          {new Date(condition.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm space-x-2">
                      ...
                      {/* ✅ Buttons now show confirmation before action */}
                      {condition.isActive ? (
                        <button
                          onClick={() => handleDeactivate(condition)}
                        ...
                        <button
                          onClick={() => handleActivate(condition)}
