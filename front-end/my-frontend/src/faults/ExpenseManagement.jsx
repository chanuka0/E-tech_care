...
//   const showSuccessMessage = (message) => {
//     const msg = document.createElement('div');
//     msg.className = 'fixed top-4 right-4 bg-green-500 text-white px-6 py-3 rounded-lg shadow-lg z-50 animate-pulse';
//     msg.textContent = message;
//     document.body.appendChild(msg);
//     setTimeout(() => msg.remove(), 3000);
//   };
...
//   const totalExpenses = filteredExpenses.reduce((sum, expense) => sum + parseFloat(expense.price), 0);
...
//               <div>
//                 <p className="text-sm text-gray-600">Average Expense</p>
//                 <p className="text-3xl font-bold text-green-600">{formatCurrency(averageExpense)}</p>
//               </div>
//               <div className="bg-green-100 p-3 rounded-full">
//                 <svg className="w-8 h-8 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
//                   <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" />
...
//                     </td>
//                     <td className="px-6 py-4 whitespace-nowrap">
//                       <span className="text-sm font-bold text-red-600">{formatCurrency(expense.price)}</span>
//                     </td>
//                     <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
//                       {new Date(expense.createdAt).toLocaleDateString('en-US', {
//                         year: 'numeric',
...
//         `http://localhost:8081/api/expenses?startDate=${startDate}&endDate=${endDate}`,
//         {
//           method: 'GET',
//           headers: {
//             'Authorization': `Bearer ${token}`,
...
const ExpenseManagement = () => {
  ...
  // Initialize date range on mount - set to 1 year ago to today
  useEffect(() => {
    const now = new Date();
    const oneYearAgo = new Date(now.getTime() - 365 * 24 * 60 * 60 * 1000);
    
    setDateRange({
      start: oneYearAgo.toISOString().slice(0, 16),
      end: now.toISOString().slice(0, 16)
    });
  }, []);

  // Fetch expenses when token or date range changes
  useEffect(() => {
  ...
  const fetchExpenses = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:8081/api/expenses?startDate=${encodeURIComponent(startDate)}&endDate=${encodeURIComponent(endDate)}`,
        {
          method: 'GET',
          headers: {
            'Authorization': `Bearer ${token}`,
          ...
          <div className="flex justify-center items-center h-64">
            <div className="text-center">
              ...
              <p className="text-gray-600 mt-1">
                {searchTerm ? 'Try adjusting your search' : 'No expenses in this date range. Adjust filter or add new expense.'}
              </p>
            </div>
          </div>
        ) : (
