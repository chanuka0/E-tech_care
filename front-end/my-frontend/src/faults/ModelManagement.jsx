...
const ModelManagement = () => {
  ...
  const handleDeleteModel = async (id) => {
    ...
    if (deletionStatus?.canDelete) {
      confirmMessage = `Are you sure you want to permanently delete the model "${model?.modelName}"? This action cannot be undone as this model is not linked to any model numbers.`;
    } else {
      confirmMessage = `The model "${model?.modelName}" is linked to model numbers and cannot be permanently deleted. It will be marked as inactive instead. Continue?`;
    }
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm space-x-2">
                        ...
                        {!canDelete && isLinked && (
                          <span 
                            ...
                            title="Cannot delete: Model is linked to model numbers"
                          >
                            🔒 Protected
                          </span>
                        )}
