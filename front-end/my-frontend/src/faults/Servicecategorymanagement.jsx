...
const ServiceCategoryManagement = () => {
  ...
  const handleAddCategory = async (newCategory) => {
    try {
      const response = await apiCall('/api/service-categories', { method: 'POST', body: JSON.stringify(newCategory) });
      setCategories([...categories, response]);
      setShowAddModal(false);
      showSuccessMessage('Service category added successfully!');
    } catch (err) { throw new Error(err.message || 'Failed to add service category'); }
  };

  const handleUpdateCategory = async (updatedCategory) => {
    try {
      const response = await apiCall(`/api/service-categories/${selectedCategory.id}`, { method: 'PUT', body: JSON.stringify(updatedCategory) });
      setCategories(categories.map(cat => cat.id === selectedCategory.id ? response : cat));
      setShowEditModal(false); setSelectedCategory(null);
      showSuccessMessage('Service category updated successfully!');
    } catch (err) { throw new Error(err.message || 'Failed to update service category'); }
  };
  ...
  const handleToggleCategory = async (category) => {
    ...
    try {
      const response = await apiCall(`/api/service-categories/${category.id}/toggle`, { method: 'PATCH' });
      setCategories(categories.map(cat => cat.id === category.id ? response : cat));
      showSuccessMessage(`Service category ${response.isActive ? 'activated' : 'deactivated'} successfully!`);
    } catch (err) { setError(err.message || 'Failed to toggle service category'); }
  };

  const handleDeleteCategory = async (id) => {
    if (!window.confirm('Are you sure you want to delete this service category? This action cannot be undone.')) return;
    try {
      await apiCall(`/api/service-categories/${id}`, { method: 'DELETE' });
      setCategories(categories.filter(cat => cat.id !== id));
      showSuccessMessage('Service category deleted successfully!');
    } catch (err) { setError(err.message || 'Failed to delete service category'); }
  };
  ...
  const activeCategories = categories.filter(c => c.isActive).length;
  const inactiveCategories = categories.filter(c => !c.isActive).length;
  // Count categories with special price set
  const specialPriceCategories = categories.filter(c => c.specialServicePrice != null && c.specialServicePrice > 0).length;
  ...
  if (!isAdmin()) {
    return (
      <div className="max-w-6xl mx-auto p-6">
        <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-lg">
          <p className="font-medium">Access Denied</p>
          <p className="text-sm mt-1">You do not have permission to access Service Category Management.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-50 pb-0">
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-6 py-6">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Service Category Management</h1>
              <p className="text-sm text-gray-600 mt-1">Manage and configure service categories with regular and special pricing</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8 pb-0">
        <div className="bg-white rounded-lg shadow mb-0">
          ...
          <div className="px-6 py-5 border-b border-gray-200 flex justify-between items-center">
            <div>
              ...
              <p className="text-sm text-gray-600 mt-1">
                Manage service categories · <span className="text-purple-600 font-medium">⭐ {specialPriceCategories} with special pricing</span>
              </p>
            </div>
            <button
              onClick={() => setShowAddModal(true)}
          ...
          <div className="px-6 py-3 flex justify-between items-center text-sm text-gray-600">
            <span>Showing {filteredCategories.length} of {totalCategories} service categories</span>
            <button onClick={fetchCategories} className="text-blue-600 hover:text-blue-800 font-medium flex items-center space-x-1">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
              </svg>
              <span>Refresh</span>
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    {/* ✅ FIXED: Regular Price and Special Price columns now in correct position */}
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Regular Price</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-purple-600 uppercase tracking-wider">⭐ Special Price</th>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredCategories.map(category => {
                    const hasSpecial = category.specialServicePrice != null && category.specialServicePrice > 0;
                    return (
                      <tr key={category.id} className={`hover:bg-gray-50 transition-colors ${hasSpecial ? 'bg-purple-50/30' : ''}`}>
                        ...
                        <td className="px-4 py-4 whitespace-nowrap">
                          <div className="flex items-center gap-1.5">
                            <span className="text-sm font-bold text-gray-900">{category.name}</span>
                            {hasSpecial && (
                              <span className="text-xs bg-purple-100 text-purple-700 px-1.5 py-0.5 rounded-full font-medium">⭐ Special</span>
                            )}
                          </div>
                          {category.code && <p className="text-xs text-gray-400 font-mono mt-0.5">{category.code}</p>}
                        </td>
                        ...
                        <td className="px-4 py-4 whitespace-nowrap">
                          {hasSpecial ? (
                            <div>
                              ...
                              {category.servicePrice > 0 && (
                                <p className="text-xs text-gray-400 mt-0.5">
                                  Save Rs.{(category.servicePrice - category.specialServicePrice).toFixed(2)}
                                </p>
                              )}
                            </div>
                          ) : (
                        ...
                        <td className="px-4 py-4 whitespace-nowrap">
                          <div className="flex items-center space-x-2">
                            ...
                            <button
                              ...
                              className="text-blue-600 hover:text-blue-900 font-medium transition-colors inline-flex items-center gap-1 px-2 py-1 rounded hover:bg-blue-50"
                            >
                              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                              </svg>
                            ...
                            <button
                              onClick={() => handleToggleCategory(category)}
                              className={`font-medium transition-colors px-2 py-1 rounded flex items-center gap-1 text-sm ${
                                category.isActive
                                  ? 'text-yellow-600 hover:text-yellow-900 hover:bg-yellow-50'
                                  : 'text-green-600 hover:text-green-900 hover:bg-green-50'
                              }`}
          ...
          <div className="px-6 pb-6 pt-4 border-t border-gray-200">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div className="bg-white border-2 border-gray-200 rounded-lg p-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-3xl font-bold text-green-600 mt-1">{activeCategories}</p>
                    <p className="text-xs text-gray-500 mt-0.5">Available for job cards</p>
                  </div>
                  <div className="bg-green-100 p-3 rounded-full">
                    <svg className="w-6 h-6 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
              ...
              <div className="bg-white border-2 border-gray-200 rounded-lg p-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-3xl font-bold text-orange-600 mt-1">{inactiveCategories}</p>
                    <p className="text-xs text-gray-500 mt-0.5">Not available</p>
                  </div>
                  <div className="bg-orange-100 p-3 rounded-full">
                    <svg className="w-6 h-6 text-orange-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636" />
              ...
              {/* ✅ NEW: Special pricing stat card */}
              <div className="bg-white border-2 border-purple-200 rounded-lg p-4">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm text-purple-600 font-medium">⭐ With Special Price</p>
                    <p className="text-3xl font-bold text-purple-700 mt-1">{specialPriceCategories}</p>
                    <p className="text-xs text-gray-500 mt-0.5">For regular customers</p>
                  </div>
      ...
      {showAddModal && (
        <AddServiceCategoryModal onAdd={handleAddCategory} onClose={() => setShowAddModal(false)} existingCategories={categories} />
      )}
      {showEditModal && selectedCategory && (
        <EditServiceCategoryModal
          category={selectedCategory}
