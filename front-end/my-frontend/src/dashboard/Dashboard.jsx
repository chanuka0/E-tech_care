...
const Dashboard = ({ onNavigate }) => {
  ...
  // UPDATED: Changed from datetime-local (16 chars) to date only (10 chars)
  ...
  const fetchProfitReport = async () => {
    ...
    setError('');
    try {
      // UPDATED: Added time components to convert date to full datetime for API
      const startDateTime = `${dateRange.start}T00:00:00`;
      const endDateTime = `${dateRange.end}T23:59:59`;
      
      const data = await apiCall(`${API_ENDPOINTS.PROFIT_REPORT}?start=${startDateTime}&end=${endDateTime}`);
    ...
    <div className="space-y-6 p-6 bg-gray-50 min-h-screen">
      ...
      {dashboardData && (
        <>
          ...
          {/* Financial Stats - Second Row (NOW 3 COLUMNS) */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            {/* ✅ NEW: Today's Expenses - white card with red icon */}
            <div className="bg-white p-6 rounded-lg shadow-lg hover:shadow-xl transition-shadow">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-gray-600 mb-1">Today's Expenses</p>
                  <p className="text-3xl font-bold text-red-600">
                    {formatCurrency(dashboardData.dailyExpenses)}
                  </p>
                </div>
          ...
          <div className="bg-white p-6 rounded-lg shadow-lg">
            ...
            {/* UPDATED: Changed from datetime-local to date input */}
