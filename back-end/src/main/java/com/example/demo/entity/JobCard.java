...
//
//    // NEW: Direct barcode field for quick scanning
//    @Column(name = "device_barcode", length = 100)
...
public class JobCard {
    ...
    // ✅ NEW: Regular Customer Relationship
    @ManyToOne(fetch = FetchType.EAGER)
    @JoinColumn(name = "customer_id")
    private Customer customer;

    // ✅ NEW: Flag to identify if this job card uses a regular customer
    ...
    // ✅ NEW: Helper method to load customer data into job card
    public void loadCustomerData(Customer regularCustomer) {
        if (regularCustomer != null) {
            this.customer = regularCustomer;
            this.isRegularCustomer = true;
            this.customerName = regularCustomer.getCustomerName();
            this.customerPhone = regularCustomer.getPhoneNumber();
            this.customerEmail = regularCustomer.getEmail();
        }
    }
