...
//    List<JobCard> findByDeviceType(String deviceType);
//
//    // NEW: Find job cards by service category
...
public interface JobCardRepository extends JpaRepository<JobCard, Long> {
    ...
    @Query("SELECT j FROM JobCard j WHERE j.customerName LIKE %:search% OR j.customerPhone LIKE %:search% OR j.jobNumber LIKE %:search% OR j.deviceBarcode LIKE %:search%")
    List<JobCard> searchJobCards(@Param("search") String search);
}