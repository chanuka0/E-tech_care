...
public interface JobCardRepository extends JpaRepository<JobCard, Long> {
    ...
    /**
     * ✅ NEW: Sum advance payments for job cards created in a date range
     * that do NOT yet have a linked invoice (to avoid double-counting with Payment records).
     * Only count job cards that are NOT cancelled.
     * This is used by ReportService to include advances in daily/monthly revenue
     * before an invoice is created for the job card.
     */
    @Query("SELECT COALESCE(SUM(j.advancePayment), 0) FROM JobCard j " +
            "WHERE j.createdAt BETWEEN :start AND :end " +
            "AND j.status != 'CANCELLED' " +
            "AND j.advancePayment > 0 " +
            "AND NOT EXISTS (" +
            "   SELECT 1 FROM Invoice i " +
            "   WHERE i.jobCard.id = j.id " +
            "   AND i.isDeleted = false" +
            ")")
    Double sumAdvancePaymentsWithoutInvoice(
