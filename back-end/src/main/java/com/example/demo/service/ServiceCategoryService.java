...
public class ServiceCategoryService {
    ...
    public ServiceCategory createServiceCategory(ServiceCategory serviceCategory) {
        ...
        if (serviceCategory.getSpecialServicePrice() != null && serviceCategory.getSpecialServicePrice() < 0)
            throw new IllegalArgumentException("Special service price cannot be negative");
        ...
        if (serviceCategory.getIsActive() == null) serviceCategory.setIsActive(true);

        ServiceCategory saved = serviceCategoryRepository.save(serviceCategory);

        notificationService.sendNotification(
                NotificationType.STOCK_UPDATE,
                "Service category created: " + serviceCategory.getName() +
                        " (Rs." + serviceCategory.getServicePrice() + ")" +
                        (serviceCategory.getSpecialServicePrice() != null ?
                                " | Special: Rs." + serviceCategory.getSpecialServicePrice() : ""),
                saved, NotificationSeverity.SUCCESS);
    ...
    public ServiceCategory updateServiceCategory(Long id, ServiceCategory updates) {
        ...
        if (updates.getName() != null && !updates.getName().isBlank()) {
            ...
            if (updates.getCode() == null || updates.getCode().isBlank())
                existing.setCode(generateCodeFromName(updates.getName()));
        }
        ...
        // ✅ Update special service price (allow setting to null to remove it)
        if (updates.getSpecialServicePrice() != null) {
            if (updates.getSpecialServicePrice() < 0)
                throw new IllegalArgumentException("Special service price cannot be negative");
            existing.setSpecialServicePrice(updates.getSpecialServicePrice());
        } else {
            // Explicitly null means "remove special price"
            existing.setSpecialServicePrice(null);
        }
        ...
        notificationService.sendNotification(
                NotificationType.JOB_UPDATED,
                "Service category updated: " + existing.getName(),
                saved, NotificationSeverity.INFO);
    ...
    public ServiceCategory toggleServiceCategory(Long id) {
        ...
        notificationService.sendNotification(
                NotificationType.JOB_UPDATED,
                "Service category toggled: " + existing.getName() + " - Status: " +
                        (existing.getIsActive() ? "ACTIVE" : "INACTIVE"),
                saved, NotificationSeverity.INFO);
    ...
    public void deleteServiceCategory(Long id) {
        ...
        notificationService.sendNotification(
                NotificationType.ITEM_REMOVED,
                "Service category deleted: " + category.getName(),
                category, NotificationSeverity.WARNING);
    }
