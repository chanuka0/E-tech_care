...
public class InvoiceService {
    ...
    private void deductNonSerializedForDirectInvoice(InvoiceItem item, Invoice invoice) {
        ...
        if (invItem.getHasSerialization()) return;
    ...
    public Invoice createInvoice(Invoice invoice) {
        ...
        // ✅ FIX: For job card invoices, only create a Payment record for the
        //         NEW money received today (paidAmount - jobCard.advancePayment).
        //         The advance was already counted in revenue on the day the job card was created
        //         via sumAdvancePaymentsWithoutInvoice(). Now that the invoice exists, that
        //         query will exclude this job card, so we must NOT record a Payment for the
        //         advance again — only record the additional amount paid now.
        if (savedInvoice.getPaidAmount() != null && savedInvoice.getPaidAmount() > 0) {
            ...
            // Get the advance already paid on the job card (if any)
            double jobCardAdvance = 0.0;
            if (savedInvoice.getJobCard() != null && savedInvoice.getJobCard().getId() != null) {
                JobCard linkedJobCard = jobCardRepository.findById(savedInvoice.getJobCard().getId())
                        .orElse(null);
                if (linkedJobCard != null && linkedJobCard.getAdvancePayment() != null) {
                    jobCardAdvance = linkedJobCard.getAdvancePayment();
                }
            }

            // Only the NEW money (above the advance) needs a Payment record today.
            // The advance was already reflected in revenue via sumAdvancePaymentsWithoutInvoice().
            double newMoneyReceived = savedInvoice.getPaidAmount() - jobCardAdvance;

            if (newMoneyReceived > 0) {
                // New money was paid today when invoice was created (rest payment)
                Payment payment = createPaymentRecord(savedInvoice, newMoneyReceived,
                        savedInvoice.getPaymentMethod(), "Payment on invoice creation (excl. prior advance)");
                if (savedInvoice.getPayments() == null) {
                    savedInvoice.setPayments(new ArrayList<>());
                }
                savedInvoice.getPayments().add(payment);
                System.out.println("💰 Invoice created — new money recorded: Rs." + newMoneyReceived
                        + " (paidAmount=" + savedInvoice.getPaidAmount()
                        + ", priorAdvance=" + jobCardAdvance + ")");
            } else if (newMoneyReceived == 0 && jobCardAdvance > 0) {
                // The paidAmount exactly equals the advance — no new money today.
                // No Payment record needed; the advance already counted via the JC query.
                System.out.println("💰 Invoice created — no new money (advance covers full paidAmount)."
                        + " paidAmount=" + savedInvoice.getPaidAmount()
                        + ", priorAdvance=" + jobCardAdvance);
            } else if (jobCardAdvance == 0) {
                // No prior advance — this is a fresh payment, record the full amount
                ...
                savedInvoice.getPayments().add(payment);
                System.out.println("💰 Invoice created — full payment recorded: Rs."
                        + savedInvoice.getPaidAmount() + " (no prior advance)");
            }
    ...
    public Invoice createDirectInvoice(CreateInvoiceRequest request) {
        ...
        // Direct invoice — no prior job card advance, record full paidAmount as Payment
        if (savedInvoice.getPaidAmount() != null && savedInvoice.getPaidAmount() > 0) {
            savedInvoice.setFirstPaymentDate(LocalDateTime.now());
            savedInvoice.setLastPaymentDate(LocalDateTime.now());
            if (savedInvoice.getPaymentStatus() == PaymentStatus.PAID)
    ...
    public Invoice createInvoiceFromJobCard(Long jobCardId, CreateInvoiceRequest request) {
        ...
        // ✅ FIX: Only create a Payment record for the NEW money received today.
        //         The job card advance was already counted in revenue on the day it was
        //         collected (via sumAdvancePaymentsWithoutInvoice). Now that the invoice
        //         exists, that query excludes this job card. So we must only record
        //         a Payment for (paidAmount - jobCard.advancePayment).
        if (savedInvoice.getPaidAmount() != null && savedInvoice.getPaidAmount() > 0) {
            savedInvoice.setFirstPaymentDate(LocalDateTime.now());
            savedInvoice.setLastPaymentDate(LocalDateTime.now());
            if (savedInvoice.getPaymentStatus() == PaymentStatus.PAID)
            ...
            // How much was already paid as advance on the job card?
            double priorAdvance = jobCard.getAdvancePayment() != null ? jobCard.getAdvancePayment() : 0.0;

            // Only the amount above the prior advance is new money today
            double newMoneyToday = savedInvoice.getPaidAmount() - priorAdvance;

            if (newMoneyToday > 0) {
                Payment payment = createPaymentRecord(savedInvoice, newMoneyToday,
                        savedInvoice.getPaymentMethod(),
                        "New payment on invoice creation (prior advance: Rs." + priorAdvance + ")");
                if (savedInvoice.getPayments() == null) savedInvoice.setPayments(new ArrayList<>());
                savedInvoice.getPayments().add(payment);
                System.out.println("💰 Job card invoice created — new money recorded: Rs." + newMoneyToday
                        + " (paidAmount=" + savedInvoice.getPaidAmount()
                        + ", priorAdvance=" + priorAdvance + ")");
            } else {
                // paidAmount <= advance — no new money today, nothing to record
                System.out.println("💰 Job card invoice created — no new money today."
                        + " paidAmount=" + savedInvoice.getPaidAmount()
                        + ", priorAdvance=" + priorAdvance);
            }
        ...
        // NO non-serialized deduction here — already deducted at job card creation
    ...
    // ========== addPayment — unchanged, always records exact amount paid ==========
    ...
    public Invoice addPayment(Long invoiceId, Double amount, PaymentMethod method) {
        ...
        // addPayment always records exactly what was paid — no adjustment needed here
        // because the advance deduction is handled at invoice creation time
