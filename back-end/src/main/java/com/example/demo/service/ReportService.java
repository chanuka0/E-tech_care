...
public class ReportService {
    ...
    /**
     * ✅ UPDATED: getDashboardStats — daily revenue now sums actual payments received today
     * (via Payment.paymentDate), so advance payments and partial payments are included
     * on the day they are received rather than only when the invoice is fully paid.
     */
    public Map<String, Object> getDashboardStats() {
        LocalDateTime now = LocalDateTime.now();
        LocalDateTime startOfDay = now.toLocalDate().atStartOfDay();
        ...
        // ============ DAILY STATISTICS ============
        // ✅ FIX: Use actual payments received today (Payment.paymentDate)
        // This includes advance/partial payments on the day they are received,
        // not just fully-paid invoices on fullyPaidDate.
        Double dailyRevenue = paymentRepository.getTotalPaymentsByDateRange(startOfDay, endOfDay);
        if (dailyRevenue == null) dailyRevenue = 0.0;
        stats.put("dailyRevenue", dailyRevenue);
        ...
        // ============ MONTHLY STATISTICS ============
        // ✅ Monthly revenue also uses actual payments received this month
        Double monthlyRevenue = paymentRepository.getTotalPaymentsByDateRange(startOfMonth, endOfMonth);
        if (monthlyRevenue == null) monthlyRevenue = 0.0;
        stats.put("monthlyRevenue", monthlyRevenue);
