...
//     * REQUIRES_NEW so it always runs in its own writable transaction,
//     * even when called from a readOnly method.
//     */
...
//
//    /**
//     * Update customer summary with latest data from job cards and invoices.
//     */
//    @Transactional
...
//     * No readOnly here — getOrCreateSummary() may need to INSERT.
//     */
//    @Transactional
...
public class CustomerSummaryService {
    ...
    /**
     * Clean up duplicate summaries for a specific customer
     */
    @Transactional
    private void cleanupDuplicatesForCustomer(Long customerId) {
        try {
    ...
    /**
     * Get or create customer summary - with duplicate cleanup
     */
    @Transactional(propagation = Propagation.REQUIRES_NEW)
    public CustomerSummary getOrCreateSummary(Long customerId) {
        Customer customer = customerRepository.findById(customerId)
    ...
    /**
     * Get detailed summary with job cards and invoices
     */
    @Transactional
    public Map<String, Object> getDetailedSummary(Long customerId) {
        Customer customer = customerRepository.findById(customerId)
        ...
        // Update summary (this will also clean duplicates)
        CustomerSummary summary = updateSummary(customerId);

        // Get job cards and invoices
        List<JobCard> jobCards = jobCardRepository.findByCustomerId(customerId);
        List<Invoice> invoices = invoiceRepository.findByCustomerPhone(customer.getPhoneNumber());
