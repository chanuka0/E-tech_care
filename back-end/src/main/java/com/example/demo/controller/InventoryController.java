...
//            return ResponseEntity.badRequest().body("Notes/reason is required for stock correction");
//        }
//
//        inventoryService.correctStock(id, quantity, serialNumbers, notes);
//        return ResponseEntity.ok(Map.of("message", "Stock correction applied successfully"));
//    }
//
...
public class InventoryController {
    ...
    /**
     * GET /api/inventory — returns only VISIBLE (non-hidden) items.
     * Used by Job Card Edit, Invoice Creation, and the main Inventory list.
     */
    @GetMapping
    @PreAuthorize("hasAnyRole('ADMIN', 'USER')")
    public ResponseEntity<List<InventoryItem>> getAllItems() {
    ...
    /**
     * GET /api/inventory/hidden — returns only HIDDEN items.
     * Used by the Hidden Archive view in Inventory Management.
     */
    @GetMapping("/hidden")
    @PreAuthorize("hasAnyRole('ADMIN', 'USER')")
    public ResponseEntity<List<InventoryItem>> getHiddenItems() {
        return ResponseEntity.ok(inventoryService.getHiddenItems());
    }
    ...
    /**
     * PATCH /api/inventory/{id}/hide — hides an item from active workflows.
     * Both ADMIN and USER can hide items.
     */
    @PatchMapping("/{id}/hide")
    @PreAuthorize("hasAnyRole('ADMIN', 'USER')")
    public ResponseEntity<InventoryItem> hideItem(@PathVariable Long id) {
        return ResponseEntity.ok(inventoryService.hideItem(id));
    }

    /**
     * PATCH /api/inventory/{id}/unhide — restores a hidden item to active workflows.
     * Both ADMIN and USER can unhide items.
     */
    @PatchMapping("/{id}/unhide")
    @PreAuthorize("hasAnyRole('ADMIN', 'USER')")
    public ResponseEntity<InventoryItem> unhideItem(@PathVariable Long id) {
        return ResponseEntity.ok(inventoryService.unhideItem(id));
    }
